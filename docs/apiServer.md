...
**クエリパラメータ（ページング・絞り込み。すべて省略可）**

| パラメータ | 説明 |
|-----------|------|
| `page` | 1始まりのページ番号。指定するとページング結果を返す |
| `pageSize` | 1ページあたりの件数（省略時は `glint.json` の `documentsPageSize`、デフォルト50） |
| `tag` | 完全一致で絞り込むタグ |
| `search` | 文書名の部分一致検索文字列 |

`page`/`pageSize`/`tag`/`search` のいずれかを指定すると、レスポンス形式が変わり `summaries`/`documents` テーブルをSQL側でJOIN・LIMIT/OFFSETしたページング結果を返す（文書数が多いグループでも全件をメモリにロードしない）。

**レスポンス例（`?page=1&pageSize=2&tag=プログラム`）**
```json
{
  "total": 5,
  "page": 1,
  "pageSize": 2,
  "documents": [
    { "name": "readme", "url": "...", "time": 1732000000000, "tag": "プログラム", "category": ["ドキュメント"] },
    { "name": "doc3", "url": "...", "time": 1732000002000, "tag": "プログラム", "category": null }
  ]
}
```

`total` は絞り込み後の全件数（`documents` は現在のページの件数のみ）。
