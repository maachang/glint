...
`tags` が配列でない場合は **`400`**。既存グループが存在しない場合は **`500`**（`.vss`ファイルが未作成のため）。
...
`message` は回答本文（Markdown記法可）、`list` は回答内で実際に引用された参考文書（`name`/`url`）の配列。参照文書のリンク表示（Markdownの `[name](url)` 形式への組み立てなど）はAPI側では行わず、クライアント側（`public/js/app.js`）の責務とする（文書名/URLに括弧等の記号が含まれる場合でも、クライアント側で正確に組み立てることでMarkdownリンク記法の崩れを避けられる）。
...
- **タグ/カテゴリフィルタは事後フィルタ**: 上記「POST /api/groups/:group/search」の注意事項を参照
