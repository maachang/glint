...
- `src/metaStore.js` は `node:sqlite`（Node.js 22.5+の実験的機能。これに伴い必須Nodeバージョンを22.5以上に上げた）または`bun:sqlite`（Bun実行時）を使い、`.vgs`/`.vss`とは独立したSQLiteデータ（`vectorStorePath`配下の`glint.sqlite`、`.gitignore`済み）を管理する。用途は3つ: (1) 文書のtag/category集計（`documents`テーブル、`getGroupStats`をSQL化して高速化）, (2) チャンクの全文検索インデックス（`chunk_fts`, FTS5, ハイブリッド検索用）, (3) 検索ログ（`search_log`, `searchLogEnabled`設定でデフォルトOFFのopt-in、質問文・引用文書一覧を記録）。このモジュール導入前から存在するグループにはSQL側データが無いため、`backfill_status`テーブルで管理しつつ初回アクセス時に`.vgs`/`.vss`から遅延的に一括構築する（利用者側の移行作業は不要）
