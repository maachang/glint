...
- `src/apiServer.js`（Node標準httpのみ、外部依存なし）でHTTP API化しており、文書登録・RAG検索・グループ一覧/文書一覧/タグカテゴリ集計などをAPI経由で利用できる
...
| src/apiServer.js | 文書登録・RAG検索・グループ管理を提供するHTTP APIサーバー(Node標準httpのみ、外部依存なし) | 
