...
| src/connectMan.js | 接続情報を管理. | 
| src/conv.js | 各種変換処理. | 
| src/llamaCpp.js | llamaCpp(openAIのAPI互換にアクセスするための実装). | 
| src/prompt.js | config.js から切り離してシステムプロンプト、ユーザプロンプトを分けるためのもの | 
| src/sync.js | 複数のLocalLLM(llamaCpp)に接続管理を行うために必要な同期処理を行うもの | 
| src/util.js | ユーティリティ. | 
| src/vectorGroup.js | RAGの文書情報をベクトルDBに保存・管理するためのもの| 
...
# 未対応・残課題(随時更新)
- 一応動く形だけど、現状のものをシステムプロンプト、ユーザプロンプトに分けて処理するための実装(prompt.js)に分けようとしている所
-
